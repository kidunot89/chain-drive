...
import { reduce, omit, each } from 'lodash';
...
var processTimeout = function processTimeout(rootEl, params) {
  if (params) {
    if (Array.isArray(params)) {
      return reduce(params, function (highestTimeout, nextParams) {
  ...
  function (_React$Component) {
    ...
    _createClass(AnimeJs, [{
      ...
      value: function animateTimeline(status) {
        ...
        var rootEl = this.ref.current;
        each(this.props[status], function (params) {
      ...
      key: "render",
      value: function render() {
        var props = omit(this.props, ['initial', 'entering', 'exiting', 'processTimeout']);
